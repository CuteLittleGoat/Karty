...
- W układach z panelami bocznymi (np. `Lata / środek / Ranking` oraz `Sekcja / środek`) szerokość paneli jest dopasowywana do kontekstu. W `Main` dla `Gry admina` panel `Lata` jest nieco węższy, a panel `Ranking` szerszy, aby ograniczyć przewijanie tabeli rankingu.
