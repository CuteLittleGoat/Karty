...
- W nagłówku panelu użytkownika (Main i Second) dodano grupę `.user-panel-refresh-controls` z przyciskiem `Odśwież` (`.secondary`) i tekstem statusu (`.status-text`); grupa jest wyrównana inline i ma minimalną szerokość statusu `160px`, żeby komunikat nie powodował skakania układu.
