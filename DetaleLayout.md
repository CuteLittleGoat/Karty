...
- W module Main przyciski zbiorczej zmiany wag (`.admin-weight-bulk-button`) w tabelach statystyk zakładek **Gry admina** i **Statystyki** mają stałą szerokość `8ch` (`width/min-width/max-width`), dzięki czemu etykiety `Waga1…Waga6` mieszczą się i nie poszerzają kolumn.
...
- W module Main modal „Szczegóły gry” (`.game-details-modal-card`) ma układ flex w `.modal-body`, a sekcja `.admin-table-scroll` ma aktywne przewijanie pion/poziom z inercją dotykową, więc długa lista graczy pozostaje edytowalna bez rozsuwania okna poza viewport.
