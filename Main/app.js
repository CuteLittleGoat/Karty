...
const initAuthControls = () => {
  ...
  const getFirebaseErrorHint = (error, contextLabel) => {
    ...
    if (code === "auth/user-not-found") {
      return "Nie znaleziono konta dla podanego e-maila.";
    }
    if (code === "auth/wrong-password") {
      return "Podane hasło jest nieprawidłowe.";
    }
    if (code === "auth/invalid-credential") {
      return "Dane logowania są nieprawidłowe — sprawdź e-mail i hasło.";
    }
    if (code === "auth/network-request-failed") {
      return "Nie udało się połączyć z Firebase. Sprawdź połączenie z internetem i konfigurację Firebase.";
    }
