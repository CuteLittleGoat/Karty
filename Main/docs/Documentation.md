...
- W modalach szczegółów gry ustawiono pionowe przewijanie zarówno dla kontenera treści (`.game-details-modal-card .modal-body`), jak i dla sekcji tabeli (`.game-details-modal-card .admin-table-scroll` z `max-height`), dzięki czemu przy dużej liczbie graczy można przewinąć listę i edytować wszystkie wiersze.
