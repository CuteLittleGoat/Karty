...
- Akcje `Dodaj Rebuy` i `Usuń Rebuy` są obsługiwane w delegowanym `click` modala (wspólnie z zamykaniem), z warunkami per `data-role`, dzięki czemu przyciski działają stabilnie na desktopie i mobile, a po wykonaniu zapisują stan oraz odświeżają widok.
