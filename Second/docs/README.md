...
- Jeśli dane sekcji `Podział puli` są chwilowo niespójne, panel pokaże czytelny komunikat błędu zamiast pozostawienia poprzedniej zakładki na ekranie.
...
10g. Jeśli gracz ma poprawnie nadane uprawnienia i widzi przyciski sekcji w sidebarze, aplikacja renderuje dane tych sekcji bez komunikatu „Brak dostępnych paneli...”; lista dozwolonych sekcji do renderu jest pobierana bezpośrednio z wyniku filtrowania przycisków sidebaru.
...
3. `Tabela16`:
   ...
   - format procentowy dla wierszy 1–3 (`xx%`) jest renderowany tak samo w panelu admina i w widoku user (po PIN), więc sekcja nie powinna już zgłaszać błędu `formatPercentDisplay is not defined`,
