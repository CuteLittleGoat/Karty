...
- Legacy pole `notes` jest usuwane z dokumentów gier przy odczycie listy (`onSnapshot`) oraz podczas zapisu notatek, przez `FieldValue.delete()`.
- Nie ma fallbacku do legacy pola `notes` — aplikacja działa wyłącznie na `preGameNotes` i `postGameNotes`.
