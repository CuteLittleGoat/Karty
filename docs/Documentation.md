...
5. Każda kolumna `Rebuy1..RebuyN` ma stałą szerokość `8ch` — dokładnie tyle jako `min-width`, `width` i `max-width`.
6. Każda zmiana w polach rebuy natychmiast odświeża obliczenia w tabelach 1, 3, 5.
7. Przed przebudową tabeli modala zapisywany jest stan fokusu (`data-focus-target`, `data-section`, `data-table-id`, `data-row-id`, `data-column-key`) i po renderze przywracany przez `restoreFocusedAdminInputState(...)`, co zabezpiecza przed utratą fokusu przy autozapisie/re-renderze.
